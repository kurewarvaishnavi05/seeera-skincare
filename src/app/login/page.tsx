...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Error details: ' + (err.message || String(err)));
    } finally {
      setLoading(false);
    }
  };
    ...
    <main className="pt-32 pb-24 min-h-screen bg-[#F8F5F2] flex flex-col items-center justify-center px-6">
      ...
      >
        <div className="text-center mb-10">
          <h1 className="text-3xl font-heading text-primary-brown mb-2">Welcome Back</h1>
          <p className="text-dark-brown/70 font-light text-sm">Please sign in to your Seeera account</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <a href="#" className="text-xs text-accent-brown hover:text-primary-brown transition-colors">Forgot Password?</a>
            </div>
            <input 
              type="password" 
              required
        ...
        <div className="mt-8 text-center text-sm text-dark-brown/70 font-light">
          ...
          <Link href="/register" className="text-primary-brown font-medium hover:text-accent-brown transition-colors">
            Create an account
          </Link>
        </div>
      </motion.div>
